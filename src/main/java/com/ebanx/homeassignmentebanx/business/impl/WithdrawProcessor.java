...
public class WithdrawProcessor implements TransactionProcessor {
	...
	public Map<String, Object> process(TransactionRequest transactionRequest) {
		Account account = accountService.findById(transactionRequest.getOrigin())
				.orElseThrow(() -> new AccountNotFoundException());
		account.withdraw(transactionRequest.getAmount());
		accountService.save(account);
		
		Transaction withdraw = TransactionMapper.mapToNewTransaction(transactionRequest);
		transactionService.save(withdraw);
		
		return ResponseUtils.buildWithdrawReponse(account);
	}
